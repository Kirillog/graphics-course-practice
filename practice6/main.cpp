...
void main()
{
    ...
    if (mode == 1) {
        ...
        color = texture(render_result, texcoord + vec2(sin(texcoord.y * 50.0 + time) * 0.01, 0.0));
    } else if (mode == 3) {
        vec4 sum = vec4(0.0);
        float sum_w = 0.0;
        const int N = 7;
...
int main() try {
    ...
    while (running) {
        ...
        std::array<float, 4> red = {1.f, 0.f, 1.f, 0.f},
                             green = {0.f, 1.f, 1.f, 0.f},
                             blue = {0.f, 0.f, 0.f, 0.f},
                             alpha = {1.f, 1.f, 1.f, 1.f};

        std::array<glm::vec3, 4> angles = {
            glm::vec3(0, 0, 0),
            glm::vec3(0, -1, 0),
            glm::vec3(0, 0, 0),
            glm::vec3(1, 0, 0),
        ...
        for (size_t i = 0; i < 4; ++i) {
            ...
            glClearColor(red[i], green[i], blue[i], alpha[i]);
            ...
            if (i) {
                projection = glm::ortho<float>(-aspect_ratio, aspect_ratio,
                                               -1.0, 1.0, near, far);
                if (i != 2) {
                    view = glm::rotate(view, glm::pi<float>() / 2.f, angles[i]);
                }
            }

            glm::vec3 camera_position =
                (glm::inverse(view) * glm::vec4(0.f, 0.f, 0.f, 1.f));
