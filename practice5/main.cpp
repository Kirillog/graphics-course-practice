...
void main()
{
    ...
    vec2 cord = vec2(texcoord.x + cos(time), texcoord.y + sin(time));
    vec4 data = texture(sampler, cord);
...
void generateMipmaps(size_t size) {
    size >>= 1;
    glGenerateMipmap(GL_TEXTURE_2D);
    std::uint32_t color = 0x000000FFu;
    for (int level = 1; level <= 3; ++level, size >>= 1, color <<= 8) {
        std::vector<std::uint32_t> pixels(size * size, color | 0xFF000000u);
        glTexImage2D(GL_TEXTURE_2D, level, GL_RGBA8, size, size, 0, GL_RGBA,
...
int main() try {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 sizeof(obj_data::vertex) * cow.vertices.size(),
                 cow.vertices.data(), GL_STATIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(uint32_t) * cow.indices.size(),
    ...
    GLuint chess_texture, cow_texture;
    ...
    for (size_t x = 0; x < size; ++x) {
        for (size_t y = 0; y < size; ++y) {
            pixels[x * size + y] = ((x + y) & 1) ? 0xFF000000u : 0xFFFFFFFFu;
        }
    }
    ...
    uint8_t *data = stbi_load(
        cow_texture_path.data(), reinterpret_cast<int *>(&size),
        reinterpret_cast<int *>(&size), &channels_in_file, channels_in_file);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, size, size, 0, GL_RGBA,
